 ...
 /// <summary>
    ...
    private SwapTransactionResponse MapToResponse(SwapTransaction swap)
    {
        return new SwapTransactionResponse
        {
            ...
            PaymentType = (swap.PaymentId.HasValue ? PaymentType.PayPerSwap.ToString() : PaymentType.Subscription.ToString()),
            SwapFee = swap.Payment?.Amount ?? 0,
            TotalAmount = swap.Payment?.Amount ?? 0,
            IsPaid = swap.PaymentId.HasValue ? (swap.Payment != null && swap.Payment.Status == PaymentStatus.Completed) : true,
